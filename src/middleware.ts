  ...
  if (token) {
    ...
    if (pathname === "/") {
      ...
      if (token.role === "ADMIN") {
        ...
        console.log("🔍 Developer redirecting to dashboard-user (status:", approvalStatus, ")");
        return NextResponse.redirect(new URL("/dashboard-user", request.url));
      }
    }
...
export const config = {
  matcher: [
    // Exclude all Next.js internals and static assets, only run on actual app routes
