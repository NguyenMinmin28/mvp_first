...
export default function IdeaSparkAdminPage() {
  ...
  const fetchData = async () => {
    try {
      // Fetch pending ideas
      const pendingResponse = await fetch('/api/admin/ideas/pending?limit=200&ts=' + Date.now(), { cache: 'no-store' });
      ...
      const approvedResponse = await fetch('/api/ideas?status=APPROVED&limit=20', { cache: 'no-store' });
      ...
      // Fetch reports
      const reportsResponse = await fetch('/api/admin/reports', { cache: 'no-store' });
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-2xl font-bold text-gray-900">IdeaSpark Management</h1>
          <p className="text-gray-600">Manage community ideas, approvals, and reports</p>
        </div>
      </div>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {approvedIdeas.reduce((sum, idea) => sum + idea.likeCount + idea.commentCount + idea.bookmarkCount + idea.connectCount, 0)}
            </div>
            <p className="text-xs text-muted-foreground">Likes, comments, bookmarks</p>
          </CardContent>
        </Card>
      </div>
                    ...
                    <div key={idea.id} className="border rounded-lg p-4 space-y-3">
                      <div className="flex items-start justify-between">
                        <div className="flex-1">
                          ...
                          <div className="flex items-center gap-2 mt-2">
                            <span className="text-sm text-gray-500">by {idea.author.name || idea.author.email}</span>
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <Button
                          size="sm"
                          variant="destructive"
                          onClick={() => handleRejectIdea(idea.id, 'Content does not meet guidelines')}
                        >
                      ...
                      <div className="flex items-start justify-between">
                        <div className="flex-1">
                          ...
                          <div className="flex items-center gap-4 mt-2 text-sm text-gray-500">
                            <span>by {idea.author.name || idea.author.email}</span>
                            <span>•</span>
                ...
                <div className="space-y-4">
                  {reports.map((report) => (
                    <div key={report.id} className="border rounded-lg p-4 space-y-3">
                      <div className="flex items-start justify-between">
                        <div className="flex-1">
                          <h3 className="font-semibold text-lg">Report #{report.id.slice(-6)}</h3>
                          <p className="text-gray-600 text-sm">Reason: {report.reason}</p>
                          <div className="flex items-center gap-2 mt-2">
                            <span className="text-sm text-gray-500">by {report.reporter.name || report.reporter.email}</span>
                            <span className="text-sm text-gray-500">•</span>
                            <span className="text-sm text-gray-500">
